...
async def search_objects(obj_type: str, city: str, shown: set) -> list:
    ...
    blacklist = (
        "СТРОГО ЗАПРЕЩЕНО включать в ответ:\n"
        "- Государственные и федеральные объекты (телебашни, МГУ, Кремль, госучреждения)\n"
        "- Крупные бизнес-центры и небоскрёбы под охраной (Москва-Сити и подобные)\n"
        "- Объекты с серьёзной охраной ФСБ/ФСО/полиции\n"
        "- Известные туристические достопримечательности (ВДНХ, Останкино и подобные)\n"
        "- Действующие промышленные предприятия\n"
        "Нужны малоизвестные объекты, о которых знает узкий круг людей.\n"
    )

    if obj_type == "roof":
        task = f"""Найди 3 реальных точки для РУФИНГА (залезть на крышу многоэтажки) в городе {city}.
{exclude}{blacklist}НЕ нужны: аренда крыш, рестораны, банкеты. НУЖНЫ: обычные жилые или нежилые высотки куда залезают руферы."""
    else:
        task = f"""Найди 3 реальных ЗАБРОШЕННЫХ объекта в городе {city}.
{exclude}{blacklist}Нужны реально заброшенные здания — не работающие, малоизвестные объекты."""
