...
async def cmd_start(message: Message, state: FSMContext):
    ...
    if user:
        await message.answer(f"С возвращением, {user['name']}! Город: {user['city']}.", reply_markup=MAIN_KB)
    else:
        await message.answer("Привет! Я помогу найти заброшки, крыши и бомбари.\n\nКак тебя зовут?", reply_markup=ReplyKeyboardRemove())
...
async def reg_city(message: Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer(f"Готово! Город: {city}. Выбирай:", reply_markup=MAIN_KB)


async def send_objects(message: Message, obj_type: str, type_name: str, city: str):
    await message.answer(f"Ищу {type_name} в {city}... 🔍")
    objects = await search_objects(obj_type, city)

    if not objects:
        await message.answer("Ничего не нашёл. Попробуй позже.")
        return
...
async def handle_zabroshka(message: Message):
    user = await get_user(message.from_user.id)
    if not user:
...
async def handle_roof(message: Message):
    user = await get_user(message.from_user.id)
    if not user:
        await message.answer("Сначала зарегистрируйся — напиши /start")
        return
...
async def handle_digger(message: Message):
    user = await get_user(message.from_user.id)
    if not user:
        await message.answer("Сначала зарегистрируйся — напиши /start")
        return
